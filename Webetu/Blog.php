		...
		<div id="main">
			...
			<div class="headerParallax">
 			...
 			<article>
				...
				try {
					...
					/* On prend tout les articles et un les traitent uns par uns */ 
					while ($articles = $stmt->fetch()){  ?>
						<div class="news">
							<!-- On affiche le titre la date et le contenu de l'article dans des balises -->
    						...
    						<ul>
    						<?php
    						$stmt2 = $pdo->query('SELECT id, nomAuteur, idArticle, contenu FROM commentaires ORDER BY id ASC');
    						$commentaires = $stmt2->fetchAll();

    						/*Pour chaques articles on recherche tout les commentaires tels que idArticle est egal à l'id de l'article en cours*/
    						foreach ($commentaires as $commentaire) {
    							...
    							?>
									...
									<?php
									/* On regarde si l'utilisateur est un admin ou si son nom est le meme que commentaire.nomAuteur */
							...
							echo '</ul>';
							echo '</div>';
							/* Si l'on est connecté on crée et met a disposition un formulaire pour commenter */
							if (isset($_SESSION['id']) AND isset($_SESSION['nom']) AND isset($_SESSION['classe'])){
								?>
			...
			<footer>
				<div id="optionAdmin">
 					...
 					if (isset($_SESSION['id']) AND isset($_SESSION['nom']) AND isset($_SESSION['classe'])){
 						if ($_SESSION['classe'] == 'admin' ){ /*et si c'est una admin alors on lui laisse la possibilité de modifier les articles de blog*/
